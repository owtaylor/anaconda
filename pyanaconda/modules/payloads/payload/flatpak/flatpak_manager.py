...
from pyanaconda.modules.payloads.payload.flatpak.source import FlatpakRegistrySource, FlatpakStaticSource, NoSourceError
...
class FlatpakManager:
    ...
    def set_source_repository(self, repository: RepoConfigurationData):
        """Sets an URL where Flatpak content can be found.
    ...
    def add_flatpak_ref(self, ref: str):
        """Add a ref to be preinstalled.

        :param ref: Flatpak ref to be preinstalled, in the form
            [<collection_id>:](app|runtime)/<id>/[<arch>]/<branch>
        """
        self._skip_installation = False
        self._flatpak_refs.append(ref)
    ...
    def _get_source(self):
        if self._source is None:
            if self._source_repository:
                log.debug("Using Flatpak source repository at: %s/Flatpaks", self._source_repository.url)
                self._source = FlatpakStaticSource(self._source_repository, relative_path="Flatpaks")
            else:
    ...
    def calculate_size(self, progress: ProgressReporter):
        ...
        try:
            self._download_size, self._install_size = self._get_source().calculate_size(self._flatpak_refs)
    ...
    def download(self, progress: ProgressReporter):
        ...
        try:
            self._collection_location = self._get_source().download(self._flatpak_refs, self._download_location, progress)
        except NoSourceError as e:
            log.info("Flatpak source not available, skipping: %s", e)
            self._skip_installation = True
